...
function IzKalk2f()
 ...
 Box("#PRENOS KALK DOKUMENTA IZ FIRME "+cF,10,75)
  DO WHILE .t.
    ...
    // na�i najstariju KALK koja nikad nije preno�ena (marker<>"PP")
    // -------------------------------------------------------------
    SELECT DOKS
    ...
    // potvrdi ponu�eni ili unesi broj kalkulacije idfirma-idvd-brkalk
    // ---------------------------------------------------------------
    ...
    // provjeri ima li takva kalkulacija i ako je ve� preno�ena daj upozorenje
    // -----------------------------------------------------------------------
    ...
    // po�to je utvr�eno da postoji, otvaramo KALK radi prenosa
    // --------------------------------------------------------
    SELECT 0
    USE (cDir+"KALK.DBF")
    ...
    // u DOKS stavimo marker "PP" da je kalkulacija ve� jednom preno�ena
    // -----------------------------------------------------------------
    SELECT KALK
     USE
    SELECT DOKS
