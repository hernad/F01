...
IF nVisina<nLen
 @   x2,y1+INT((y2-y1)/2) SAY IF(nGornja==1,"  ",IF(nItemNo==nLen,"���","  "))
 @ x1-1,y1+INT((y2-y1)/2) SAY IF(nGornja==1,"���",IF(nItemNo==nLen,"  ","  "))
ENDIF
...
LOCAL nVrati:=1,nTipka,i:=0,nOpc:=LEN(aOpc),nRedova:=1,p:=0
  ...
  Prozor1(nXp-2,4,nXp+1+4*nRedova,75,,"N/W","�߲��ܲ� ","N/W","W/W",0)
...
for i:=1 to 99
 if !File2(cIme+alltrim(str(i))+".ART")
