...
if fNovi .and. (_idfirma<>idfirma .or. _idvn<>idvn)
	_brnal := sljedeci_fin_nalog( _idfirma, _idvn )
     	select  pripr
endif
...
IF lSint==NIL; lSint:=.f.; ENDIF
  ...
  IF !lSint
    USE_EXCLUSIVE(PRIVPATH+"POM") NEW ALIAS ANAL
  ELSE
    USE_EXCLUSIVE(PRIVPATH+"POM") NEW ALIAS SINT
  ENDIF
...
if !sifra_za_koristenje_opcije("PVNAPVN")
 return
endif
...
function IzvodBanke()
 ...
 Box(,20,75); old_m_x := m_x; old_m_y := m_y
  ...
  nH   := fopen(cIme)
  nRBr := 0
  cBrNal := sljedeci_fin_nalog( gFirma, cIdvn )
