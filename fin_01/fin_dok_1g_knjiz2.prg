...
LOCAL nRec:=0
 PRIVATE cBrojOiN:="T"     // T,S,O
  ...
  DO WHILE !EOF()
    ...
    ? SPACE(gnLMONI)
    gpB_ON(); gp12cpi()
    ?? "ORGAN UPRAVE-SLU"+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"@")+"BA"
    gpB_OFF(); gpCOND()
    ?? SPACE(50)+"Ispla"+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"}")+"eno putem"
    gpB_ON()
    ?? " ZPP-BLAGAJNE"
    gpB_OFF()

    ? SPACE(gnLMONI)+SPACE(77)+"sa "+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"`")+"iro ra"+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"~")+"una"
...
//    ? SPACE(gnLMONI)+SPACE(77); gpI_ON()
    ...
    ?; ?; ?; ?; ?
    ? SPACE(gnLMONI)+"Kojim se odre"+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"|")+"uje da se izvr"+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"{")+"i isplata u korist "; gpI_ON()
    ?? PADC(ALLTRIM(Ocitaj(F_PARTN,idpartner,"TRIM(naz)+', '+mjesto")),57); gpI_OFF()
    ? SPACE(gnLMONI)+"                                                "+REPL("-",57)
    ?
    ? SPACE(gnLMONI)+REPL("-",105)
    ? SPACE(gnLMONI)+"na ime ra"+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"~")+"una broj "; gpI_ON()
    ?? PADC(ALLTRIM(brdok),24); gpI_OFF()
    ?? " od "; gPI_ON()
    ?? PADC(DTOC(datval),23); gPI_OFF()
    ?? " za kupljenu robu - izvr"+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"{")+"ene usluge"; gPI_ON()
    ...
    ?
    ? SPACE(gnLMONI)+REPL("-",105)
    ? SPACE(gnLMONI)+"na teret ovog organa - slu"+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"`")+"be i to:        "+ValDomaca()+" "; gPI_ON()
    ...
    ? SPACE(gnLMONI)+SPACE(32)+REPL("-",33)
    ?; ?; ?; ?
    ? SPACE(gnLMONI)+SPACE(15)+"Ra"+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"~")+"unopolaga"+IF(gKodnaS=="8",BOX_CHAR_USPRAVNO,"~")+SPACE(50)+"Naredbodavac"
    ?; ?
